...
class SwarmFarmEnv(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        if self._episode_logger is not None:
            step_context = None
            if self._episode_logger.verbose_step_logging:
                step_context = self._build_step_context(
                    dt=dt, flyable=flyable, full_coverage=full_coverage,
                    new_cells_this_step=new_cells_this_step, wind_vector=wind_vector,
                    collisions_this_step=report.counts, terminated=terminated, truncated=truncated,
                    info=info, action=action,
                )
            self._episode_logger.log_step(swarm_breakdown, info, step_context=step_context)
    ...
    def _build_step_context(self, dt, flyable, full_coverage, new_cells_this_step,
                             wind_vector, collisions_this_step, terminated, truncated, info,
                             action: np.ndarray) -> dict:
        """
        Assembles the full per-timestep simulator state consumed by
        `EpisodeLogger.log_step()` to build a publication-grade,
        independently-verifiable per-step CSV row. Only called when
        `self._episode_logger.verbose_step_logging` is True (see the
        call site in `step()`) -- deliberately kept out of the hot path
        for ordinary (non-verbose) training, since it does real work
        (per-UAV state extraction, a few numpy ops) that bulk PPO/
        Q-Learning training runs should never pay for.

        Every value returned here is a plain Python/numpy scalar or a
        list of small per-UAV dicts -- `EpisodeLogger._build_verbose_record`
        is what flattens this into pandas-ready, per-UAV-indexed CSV
        columns (uav0_x, uav0_y, ...), not this method; this method's
        only job is gathering the raw state, matching the existing
        separation between `swarm_env.py` (simulation) and
        `episode_logger.py` (how it's written to disk).

        Parameters mirror local variables already computed earlier in
        `step()` -- passed in explicitly rather than recomputed, so this
        helper never silently uses stale state.

        Parameters
        ----------
        action : np.ndarray
            The raw MultiDiscrete action array passed into this step's
            `step()` call (one ActionType ordinal per UAV, indexed by
            `uav.uav_id`, which is guaranteed to equal list position --
            see UAV construction in `reset()`). Included per-UAV in
            `uav_states` so the logged CSV can distinguish, e.g., "the
            policy chose STAY" from "the policy tried to move but was
            deflected/blocked" when diagnosing a coverage plateau --
            `new_cells_visited_this_step` alone cannot make that
            distinction, since both scenarios can produce zero new cells.
        """
        uav_states = [
            {
                "x": float(uav.position[0]), "y": float(uav.position[1]),
                "vx": float(uav.velocity[0]), "vy": float(uav.velocity[1]),
                "battery_frac": float(uav.battery_s / self._config.drone.battery_capacity_s),
                "alive": uav.alive,
                "action": int(action[uav.uav_id]),
            }
            for uav in self._uavs
        ]

        return {
            "time_seconds": self._step_count * dt,
            "coverage_fraction": info["coverage_fraction"],
            "visited_cells": int(self._visited.sum()),
            "total_flyable_cells": int(flyable.sum()),
            "new_cells_visited_this_step": new_cells_this_step,
            "uav_states": uav_states,
            "wind_vector": (float(wind_vector[0]), float(wind_vector[1])),
            "number_dynamic_obstacles": len(self._dynamic_manager.obstacles),
            "number_static_obstacles": len(self._static_obstacles),
            "collisions_this_step": collisions_this_step,  # per-step raw counts, NOT cumulative
            "valid_action_fraction": info["valid_action_fraction"],
            "mission_completed": full_coverage,
